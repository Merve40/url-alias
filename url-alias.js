...
function save(e){
    ...
    util.db.get(id).then((result)=>{
    
        if(id == aliasInput.children[0].value.trim() && result[id] == urlInput.children[0].value.trim()){
            close(id, result[id]);
            ...
            util.db.remove(id).then(()=>{
                util.db.save(aliasInput.children[0].value.trim(), urlInput.children[0].value.trim()).then(()=>{
                    ...
                    close(aliasInput.children[0].value.trim(), urlInput.children[0].value.trim());
                }).catch(e=>{
                    close(id, result[id]);    
                });
            }).catch(e=>{
            ...
            promise = util.db.save(id, urlInput.children[0].value.trim()).then(()=>{
                close(aliasInput.children[0].value.trim(), urlInput.children[0].value.trim());
            }).catch(e=>{
                close(id, result[id]);
            });
